...
const Navbar = () => {
  ...
  return (
    <motion.nav
      style={{ opacity: 1 }}
      className={`fixed top-0 w-full z-50 px-6 py-4 ${isDarkMode ? "bg-gray-950/80" : "bg-gray-50/80"} backdrop-blur-md border-b ${
    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        >
          ...
          {""}
          <span
            className={`text-lg ml-1 ${isDarkMode ? "text-white" : "text-gray-950"}`}
          >
            Time to Program
          </span>
        </motion.div>
        ...
        <div className="hidden md:flex items-center space-x-8">
          ...
          {["Home", "Skills", "About", "Contact",'Projects'].map((items) => (
            <motion.button
              key={items}
              whileHover={{ y: -2 }}
      ...
      <AnimatePresence>
        {isMenuOpen && (
          ...
          >
            {["Home", "About", "Skills", "Projects", "Contact"].map((items) => (
              <motion.button
                key={items}
                whileHover={{ x: 5 }}
                onClick={() => scrollToSection(items.toLowerCase())}
